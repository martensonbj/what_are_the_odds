...
RSpec.feature "index page", :type => :feature do
  ...
  scenario "User can see sent and pending challenges" do
    ...
    expect(page).to have_text("Sent Challenges")
    expect(page).to have_text("Pending Challenges")
    within('.sent-challenges') do
      expect(page).to have_text "Pending Challenge 1"
    end
    within('.pending-challenges') do
      expect(page).to have_text "Pending Challenge 2"
    end
  end
